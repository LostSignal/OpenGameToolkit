...
#pragma warning restore 0649
        ...
        private bool isVirtualScrollRectInitialized = false;
        ...
        public void CenterOnIndex(int index)
        {
            ...
            {
                Logger.LogError($"VirtualizedScrollRect.CenterOnIndex incountered an unknown GrowType {this.growType}", this);
            }
        }
        ...
        public void OnSceneGUI()
        {
            ...
            for (int i = 0; i < this.columnCount; i++)
            {
                ...
                {
                    Vector3 cellPosition = new(this.GetColumnX(i), this.GetRowY(j));
        ...
        private void ReplaceAllVisibleTiles()
        {
            ...
            Vector3 offscreenLocation = new(this.GetColumnX(this.activeRightColumn + 2), this.GetRowY(this.activeBottomRow + 2));
            ...
            for (int i = this.activeTopRow; i <= this.activeBottomRow; i++)
            {
                ...
                {
                    Vector2 position = new(this.GetColumnX(j), this.GetRowY(i));
        ...
        {
            this.isVirtualScrollRectInitialized = false;
            this.Initialize();
        }

        private void Initialize()
        {
            if (this.isVirtualScrollRectInitialized)
            {
                return;
            }

            this.isVirtualScrollRectInitialized = true;
            ...
            if (this.growType == GrowType.Horizontally)
            {
                Logger.LogWarningFormat("GridVirtualizer has not been tested using GrowType {0}", this.growType.ToString());
            }
        }
        ...
        private void CalculateCellStartAndBounds()
        {
            ...
            Vector3 startOffset = new(this.cellSize.x / 2.0f, -this.cellSize.y / 2.0f);
        ...
        {
            Vector3 upperLeft = new(lowerLeft.x, upperRight.y);
            Vector3 lowerRight = new(upperRight.x, lowerLeft.y);
