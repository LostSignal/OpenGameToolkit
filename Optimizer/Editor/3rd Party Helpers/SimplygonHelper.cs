...
namespace OGT
{
    ...
    public static class SimplygonHelper
    {
        ...
        private static void UpdateSimplygonHelperDefines()
        {
        ...
        private static void ReduceGameObjects(List<GameObject> gameObjects, float quality, SimplygonSettings settings)
        {
            ...
            {
                Lost.Logger.LogError("ReduceGameObjects given list of zero game objects.");
                return;
            }
            ...
            foreach (var gameObject in gameObjects)
            {
                ...
                copy.name = gameObject.GetEntityId().ToString();
            ...
            {
                Lost.Logger.LogError("Unexcpected error when reducing game objects with Simplygon.");
                return;
            }
            ...
            for (int i = 0; i < batchObject.transform.childCount; i++)
            {
                ...
                {
                    Lost.Logger.LogError($"Unable to find Simplygon Version of {original.name} with instance id {originalInstanceId}", original);
                    continue;
                }

                if (simplygonTransforms.Count > 1)
                {
                    Lost.Logger.LogError($"Found too many matching Simplygon Version of {original.name} with instance id {originalInstanceId}", original);
                    continue;
                }
                ...
                {
                    Lost.Logger.LogError($"Skipping Object {original.name}, simplygon did not produce a mesh filter for this object.", original);
                    continue;
                }
        ...
        private static GameObject SimplygonReduce(GameObject gameObject, float quality, SimplygonSettings settings)
        {
            using (Simplygon.ISimplygon simplygon = global::Simplygon.Loader.InitSimplygon(out Simplygon.EErrorCodes simplygonErrorCode, out string simplygonErrorMessage))
            {
                ...
                {
                    Lost.Logger.Log($"Simplygon Initializing failed with error code {simplygonErrorCode}");
                    return null;
                }
            }
        }
