...
#pragma warning restore 0649
        ...
        public void CalculateOctree()
        {
            ...
            Logger.Log($"Mesh Renderer Collection took {meshCollctionInMillis} milliseconds");

            if (this.meshRendererInfos.Count == 0)
            {
                Logger.LogError("No valid MeshRenders found!");
                return;
            }
            ...
            Logger.Log($"Octree Generation took {octreeGenerationInMillis} milliseconds");

            Logger.Log("Mesh Renderers: " + this.meshRendererInfos.Count);
            Logger.Log("Octree Volume: " + this.octreeVolumes.Count);

            //// Lost.Logger.Log("Average MeshRender per Volume: Not Implemented");
            //// Lost.Logger.Log("Average Triangles per Volume: Not Implemented");
            ...
            if (missingFromOctreeCount > 0)
            {
                ...
                Logger.LogError($"Mesh Renderers Without Octree Volume: {missingFromOctreeCount}");
            }
#endif
        }
            ...
            void AddOctreeVolume(List<MeshRendererInfo> meshRendererInfos, Bounds bounds)
            {
                ...
                foreach (var meshRendererInfo in meshRendererInfos)
                {
                    if (meshRendererInfo.OctreeVolumeIndex != -1)
                    {
                        ...
                        Logger.LogError(errorMessage, meshRendererInfo.MeshRenderer);
                    }
                    else
                    {
